...
export class DucoSensorAccessory {
  private humidityService: InstanceType<typeof this.platform.Service.HumiditySensor> | null = null;
  private temperatureService: InstanceType<typeof this.platform.Service.TemperatureSensor> | null = null;
  private airQualityService: InstanceType<typeof this.platform.Service.AirQualitySensor> | null = null;
  private infoService;

  private currentHumidity = 0;
  private currentTemperature = 20;
  private currentCO2 = 400;
  ...
  constructor(
    ...
    this.infoService
      ...
      .setCharacteristic(this.platform.Characteristic.SerialNumber,
        currentNode.General?.Ident || `Node-${currentNode.Node}`)
      .setCharacteristic(this.platform.Characteristic.FirmwareRevision,
        currentNode.General?.SwVersion || '1.0');
  ...
  private getAirQuality(): CharacteristicValue {
    ...
    if (co2 <= 400) return AQ.EXCELLENT;
    if (co2 <= 700) return AQ.GOOD;
  ...
  private hasHumidity(node: DucoNode): boolean {
  ...
  private hasTemperature(node: DucoNode): boolean {
    return (
      node.Sensor?.Temp != null ||
  ...
  private hasCO2(node: DucoNode): boolean {
    const type = (node.General?.Type || '').toUpperCase();
    return (
      node.Sensor?.CO2 != null ||
