...
export class DucoVentilationAccessory {
  private fanService;
  ...
  private currentNode: DucoNode;
  private currentMode: 'AUTO' | 'MAN1' | 'MAN2' | 'MAN3' = 'AUTO';
  private currentSpeed = 0;  // 0-100, reflects actual airflow in Auto

  constructor(
    private readonly platform: DucoPlatform,
    private readonly accessory: PlatformAccessory,
    node: DucoNode,
    private readonly api: DucoApiClient,
    private readonly config: DucoPluginConfig,
  ) {
   ...
   */
  private setModeOptimistic(mode: 'AUTO' | 'MAN1' | 'MAN2' | 'MAN3'): void {
    ...
    this.level1SwitchService.updateCharacteristic(
      this.platform.Characteristic.On,
      mode === 'MAN1',
    );
    this.level2SwitchService.updateCharacteristic(
      this.platform.Characteristic.On,
      mode === 'MAN2',
    );
    this.level3SwitchService.updateCharacteristic(
      this.platform.Characteristic.On,
      mode === 'MAN3',
    );
  }
  ...
  private getFilterStatus(): CharacteristicValue {
   ...
   */
  private sendModeToApi(mode: string): void {
    this.setDucoMode(mode).catch((err) => {
      this.platform.log.warn(
        `API call for mode ${mode} failed — will correct on next poll:`,
        (err as Error).message,
      );
    });
  }

  private async setDucoMode(mode: string): Promise<void> {
