...
export class DucoSensorAccessory {
  ...
  private ventilationStateService: Service;
   ...
   */
  private mapIaqToHomeKit(iaq: number): number {
    if (iaq <= 0) return 0;  // Unknown
    if (iaq <= 20) return 1; // Excellent
    if (iaq <= 40) return 2; // Good
    if (iaq <= 60) return 3; // Fair
    if (iaq <= 80) return 4; // Inferior
    return 5;                // Poor
  }
  ...
  updateFromNode(node: DucoNode): void {
    ...
    this.ventilationStateService.updateCharacteristic(
